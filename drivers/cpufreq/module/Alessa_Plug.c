...
static inline void offline_cpu(void)
{
	...
	case 1:
		if(suspend_cpu_num >= NR_CPUS / 3)
			suspend_cpu_num = NR_CPUS / 3;
	break;
	case 2:
		if(NR_CPUS >= 6 && suspend_cpu_num > NR_CPUS / 6)
			suspend_cpu_num = NR_CPUS / 6;
	break;
	default:
	break;
}
...
static inline void cpu_online_all(void)
{
	...
	switch(endurance_level){
		...
		break;
	case 1:
		if(resume_cpu_num > (NR_CPUS / 3) - 1 || resume_cpu_num == 3)
		resume_cpu_num = ((NR_CPUS / 3) - 1);
	break;
	case 2:
		if(NR_CPUS >= 6 && resume_cpu_num > ((NR_CPUS / 6) -1))
			resume_cpu_num = ((NR_CPUS / 6) -1);
	break;
	default:
	break;
	}
